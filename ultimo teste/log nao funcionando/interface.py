...
def mostrar_dados_usuario(dados):
    ...
    janela_dados = root  # Agora usaremos a janela principal como "página"
...
def fazer_login():
    ...
    if bd.conn:
        ...
        cursor.execute("SELECT nome, cpf, bloco, numero_ap, email FROM Pessoa WHERE login = ? AND senha = ?", (login, senha))
        dados = cursor.fetchone()
        if dados:
